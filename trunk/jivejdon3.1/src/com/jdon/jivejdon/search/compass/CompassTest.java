package com.jdon.jivejdon.search.compass;
...
public class CompassTest extends TestCase {
	...
	protected void init() throws Exception {
		  CompassConfiguration config = new CompassAnnotationsConfiguration().configure(
	...
	public void setUpData() throws Exception {
		...
		myMessage.setSubject("EJB3与EJB2架构对比");
		StringBuffer sb = new StringBuffer();
		sb
				.append("据路透社报道，印度尼西亚社会事务部一官员星期二(29日)表示，日惹市附近当地时间27日晨5时53分发生的里氏6.2级地震已经造成至少5427人死亡，20000余人受伤，近20万人无家可归");
		sb
				.append("SUN对J2EE方案进行了定义（即J2EE规范），在J2EE1.4采用了分层体系，提出了容器和构件的概念，并明确了容器的职责、构件的职责及如何一齐协调运作，它在其中运用了JSP、XML、EJB、JTA、JDBC等13种技术。");
		sb
				.append("无可非议，该解决方案能够迎合企业应用的高要求、高复杂度。所以该解决方案得到了广泛的认可，形成了潮流，出现了中间件开发和构件开发的概念。");
		sb
				.append("中间件开发商按照J2EE规范进行容器开发，如WEBSPHERE 、WEBLOGIC 、JBOSS，构件开发商按J2EE规范专心开发业务构件，然后部署到中间件中形成应用");
		myMessage.setBody(sb.toString());
	...
	public void query() throws Exception {
		...
		try {
			...
			ForumMessage messageS = (ForumMessage) session.load(
					ForumMessage.class, myMessage.getMessageId());
	...
	public void delete() throws Exception {
		...
		compassTemplate.execute(new CompassCallbackWithoutResult() {
			protected void doInCompassWithoutResult(CompassSession session)
				...
				messageS = (ForumMessage) session.get(ForumMessage.class,
	...
	public void update() throws Exception {
		...
		compassTemplate.execute(new CompassCallbackWithoutResult() {
			protected void doInCompassWithoutResult(CompassSession session)
				...
				messageS.setSubject("new EJB3与EJB2架构对比");
				// have to save it (no automatic persistance yet)
				session.save(messageS);

				messageS = (ForumMessage) session.load(ForumMessage.class,
						myMessage.getMessageId());
				assertEquals("new EJB3与EJB2架构对比", messageS.getSubject());
			}
		});
	}
	...
	public CompassHit[] find(String iquery) throws Exception {
		...
		System.out.println("Found [" + hits.getLength() + "] hits for ["
				+ query + "] query");
		System.out
				.println("======================================================");
	...
	public static void print(CompassHits hits, int hitNumber) {
		...
		System.out.println("ALIAS [" + resource.getAlias() + "] ID ["
				+ ((ForumMessage) value).getMessageId() + "] SCORE ["
